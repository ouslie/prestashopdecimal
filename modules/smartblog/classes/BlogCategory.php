...
class BlogCategory extends ObjectModel
{
	...
	public static function getRootCategory($id_lang = null)
	{
		...
		$root_category = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				...
				INNER JOIN `'._DB_PREFIX_.'smart_blog_category_lang` sbs
	...
	public static function getNameCategory($id)
	{
		...
		$sql = 'SELECT * FROM '._DB_PREFIX_.'smart_blog_category_lang pl, '._DB_PREFIX_.'smart_blog_category p 
				...
				AND p.id_smart_blog_category='.$id.'
	...
	public static function getCatLinkRewrite($id)
	{
		...
		$sql = 'SELECT pl.link_rewrite 
				...
				'._DB_PREFIX_.'smart_blog_category p
				WHERE pl.id_smart_blog_category=p.id_smart_blog_category
				AND p.id_smart_blog_category='.$id.'
				AND pl.id_lang = '.$id_lang;
	...
	public static function getCategory($active = 1, $id_lang = null)
	{
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				...
				ON(sbc.`id_smart_blog_category` = sbcl.`id_smart_blog_category`
				AND sbcl.`id_lang` = '.(int)($id_lang).')
				INNER JOIN `'._DB_PREFIX_.'smart_blog_category_shop` sbs ON sbs.id_smart_blog_category = sbc.id_smart_blog_category
				AND sbs.id_shop = '.(int) Context::getContext()->shop->id.'
				WHERE sbc.`active`= 1');
	...
	public static function getCategoryNameByPost($id_post)
	{
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				...
				WHERE p.id_smart_blog_post =  '.$id_post);
	...
	{
		$sql = 'SELECT count(id_smart_blog_post) AS count
			FROM `'._DB_PREFIX_.'smart_blog_post`
			WHERE id_category = '. $id_smart_blog_category;
	...
	public static function GetMetaByCategory($id_category, $id_lang = null)
	{
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			ON sbs.id_smart_blog_category = sbc.id_smart_blog_category
			AND sbs.id_shop = '.(int) Context::getContext()->shop->id.'
			WHERE sbc.`active`= 1 and sbc.id_smart_blog_category = '.$id_category);

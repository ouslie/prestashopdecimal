...
class TMSocialLoginVKLoginModuleFrontController extends ModuleFrontController
{
	public function initContent()
	{
		...
		$vkid = Configuration::get('TMSOCIALLOGIN_VKAPPID');
		$vkkey = Configuration::get('TMSOCIALLOGIN_VKAPPSECRET');
		$vk_redirect_url = Configuration::get('TMSOCIALLOGIN_VKREDIRECT');
		$vk_settings = 'email';

		$this->login_url = $this->context->link->getModuleLink('tmsociallogin', 'vklogin', array(), true, $this->context->language->id);
		...
		if (isset($access_token) && $access_token)
		{
			...
			$userinfo = $vk->api('users.get', array(
        		'uids'   => $user_id,
			...
			if (empty($useremail))
			{
				...
				{
					$this->redirect_uri = $this->context->link->getModuleLink('tmsociallogin', 'vkregistration',
																				array(
																					'user_id' => $user_id,
																					'user_name' => $user_name,
																					'email' => $email,
																					...
																					'gender' => $gender,
																					'back' => Tools::getValue('state')), true, $this->context->language->id);
					Tools::redirect($this->redirect_uri);
				}
				else
				{
					$this->redirect_uri = $this->context->link->getModuleLink('tmsociallogin', 'vklink',
																				array(
																					'user_id' => $user_id,
																					'user_name' => $user_name,
																					'email' => $email,
